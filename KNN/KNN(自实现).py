...
def datingClassTest():
    ...
    # 从文件中加载数据
    datingDataMat, datingLabels = file2matrix('样本集.txt')  # load data setfrom file
    # 归一化数据
...
# import numpy as np
#
# data = pd.read_csv(r'D:\代码\机器学习\样本集.txt', sep='\\t', engine='python')
# data.columns = ['飞行里程数', '消耗冰淇淋量', '游戏时间数', '等级']
# X = np.array(data[['飞行里程数', '消耗冰淇淋量', '游戏时间数']].values.tolist()).reshape(-3, 3)
# y = np.array(data['等级'].values.tolist()).reshape(-1, 1)
# # print(X)
# # print(y)
#
#
...
# neigh.fit(X, y)
#
# print(neigh.predict([[40920, 10.141740, 0.191283], [10000, 0.1, 10]]))
# print(neigh.predict_proba([[40920, 10.141740, 0.191283]]))